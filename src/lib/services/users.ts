 ...
 */
export const getContactaPerson = async <T extends unknown[] = ContactPerson[]>(): Promise<T> => {
    const { data } = await getFetch<T>("contactPersons")
    return data
}
...
export const getContactPersonById = async <T extends object = UserSession>(contactaPersonId: string): Promise<T> => {
    const { data } = await getFetch<T>(`contactaPerson/${contactaPersonId}`)
    return data
}
 ...
 */
export const getPositionSoilDataByUser = async <T extends unknown[] = PositionSoilData[]>(
    contactPersonId: string
): Promise<T> => {
    const { data } = await getFetch<T>(`contactPerson/${contactPersonId}/positionSoilData`)
    return data
}
